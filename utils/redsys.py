...
class RedsysService:
    ...
    def prepare_payment(self, reservation, request):
        amount = Decimal(str(reservation.total_price)).quantize(
        ...
        reservation.save(
            update_fields=["payment_order", "payment_amount", "payment_signature"]
        )
        return {
    ...
    def generate_numeric_order(self, length: int = 12) -> str:
        """
        Devuelve una cadena numérica de `length` dígitos
        compatible con Redsys (4-12 dígitos).
        """
